...
def get_integration_providers(db: Session, settings: Settings | None = None) -> IntegrationProviders:
    ...
    record = cfg.get()
    if not record.jira_site_url or not record.jira_service_account_email or not record.jira_api_token_encrypted:
        raise AppError(code="jira_not_configured", message="Jira integration is not configured", status_code=400)
    if not record.ado_org_url or not record.ado_pat_encrypted:
        raise AppError(code="ado_not_configured", message="Azure DevOps integration is not configured", status_code=400)

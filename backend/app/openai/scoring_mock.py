...
class MockScoringProvider:
    ...
    def score_assessment(self, context: dict[str, Any]) -> tuple[CandidateScoringAI, dict[str, Any]]:
        ...
        for domain, practice in model.ordered_practices():
            ...
            level = next((lvl.name for lvl in model.maturity_levels if abs(lvl.score - round(score)) < 0.51), "Managed")
            practice_scores.append(
                PracticeScoreAI(
                    ...
                    human_evidence="Workshop discussion referenced this practice." if state != "not_discussed" else "",
                    jira_evidence="Jira signals available where collection succeeded." if not context.get("jira_failed") else "",
                    ado_evidence="Azure DevOps signals available where collection succeeded." if not context.get("ado_failed") else "",
                    source_turn_ids=turn_ids.get(practice.key, [])[:8],
                    contradictions=[],
                    limitations=[lim for lim in limitations if practice.key in lim.lower() or "integration" in lim.lower()][:4],
                    rationale=f"Rubric-aligned draft score for {practice.name} under {influence} influence.",
                    missing_information=["More concrete examples needed"] if state in {"partial", "clarify", "not_discussed"} else [],
                    recommendation=practice.improvement_guidance[0] if practice.improvement_guidance else "",
                )
            )

        scored = [p for p in practice_scores if p.coverage_state != "not_discussed"]
        overall = round(sum(p.ai_candidate_score for p in scored) / max(len(scored), 1), 1) if scored else 1.5
        ...
        for p in sorted(practice_scores, key=lambda x: x.ai_candidate_score)[:5]:
            if p.coverage_state == "not_discussed" and len(actions) >= 3:
                continue
            domain_key = next(d.key for d, pr in model.ordered_practices() if pr.key == p.practice_key)
            horizon = "next_sprint" if p.ai_candidate_score <= 2.0 else "ninety_days" if p.ai_candidate_score <= 3.0 else "longer_term"
            ...
            actions.append(
                ImprovementActionAI(
                    title=guidance[:240],
                    observation=p.human_evidence or f"Coverage for {p.practice_key} is {p.coverage_state}.",
                    practice_key=p.practice_key,
                    domain_key=domain_key,
                    supporting_evidence=(p.jira_evidence or p.ado_evidence or "Workshop notes")[:2000],
                    ...
                    kpi=kpi[:240],
                    priority=1 if p.ai_candidate_score <= 2.0 else 2 if p.ai_candidate_score <= 3.0 else 3,
                )
            )
        ...
        result = CandidateScoringAI(
            ...
            confidence_summary="High" if len(scored) >= 10 else "Medium" if len(scored) >= 6 else "Low",
